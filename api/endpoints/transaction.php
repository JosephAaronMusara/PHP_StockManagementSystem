...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $response['action'] = 'Get transaction';
            $response['received'] = ['id' => $_GET['id']];
            $response['data'] = $transaction->getTransactionById($_GET['id']);
            if ($response['data']) {
    ...
    case 'POST':
        ...
        $response['action'] = 'Add transaction';
        $response['received'] = $requestBody;
        $createResponse = $transaction->addTransaction($requestBody);
    ...
    case 'DELETE':
        $id = $_GET['id'] ?? null;
        if (!$id) {
            $response = ['success' => false, 'message' => 'transaction ID required.'];
            break;
        }
        $response['action'] = 'Delete transaction';
        $response['received'] = ['id' => $id];
        $deleteResponse = $transaction->deleteTransaction($id);
