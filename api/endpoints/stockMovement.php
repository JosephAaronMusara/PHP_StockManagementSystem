...
include '../core/StockMovement.php';
require '../core/Database.php';

$method = $_SERVER['REQUEST_METHOD'];
$db = new Database();
$stock_mvnt = new StockMovement($db->getConnection());
$response = [];
...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $response['action'] = 'Get stock_mvnt';
            $response['received'] = ['id' => $_GET['id']];
            $response['data'] = $stock_mvnt->getStockMovementById($_GET['id']);
            if ($response['data']) {
                $response['success'] = true;
            } else {
                $response['success'] = false;
                $response['message'] = 'stock_mvnt not found.';
            }
        } else {
            $response['action'] = 'Get All stock_mvnts';
            $response['received'] = [];
            $response['data'] = $stock_mvnt->getAllStockMovements();
            $response['success'] = true;
        }
        break;
    ...
    case 'POST':
        ...
        $response['action'] = 'Add stock_mvnt';
        $response['received'] = $requestBody;
        $createResponse = $stock_mvnt->addStockMovement($requestBody);
    ...
    case 'DELETE':
        $id = $_GET['id'] ?? null;
        if (!$id) {
            $response = ['success' => false, 'message' => 'stock_mvnt ID required.'];
            break;
        }
        $response['action'] = 'Delete stock_mvnt';
        $response['received'] = ['id' => $id];
        $deleteResponse = $stock_mvnt->deleteStockMovement($id);
